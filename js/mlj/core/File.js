...
(function () {
    ...
    function loadMeshDataFromBuffer(buffer, mf, onLoaded) {
        ...
        // Emscripten need an ArrayView, so from the returned ArrayBuffer we must create a view of it as 8bit chars
    ...
    this.openMeshFile = function (toOpen) {
        var fileList = toOpen instanceof FileList ? toOpen : [toOpen];

        fileList.forEach(function (file) {
            console.time("Read mesh file");

            if (!(file instanceof File)) {
                console.error("MLJ.file.openMeshFile(file): the parameter 'file' must be a File instance.");
                return;
            }
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                loadMeshDataFromBuffer(buffer, mf, function (loaded, meshFile) {
                    if (loaded) {
                        // console.log("Mesh file loaded, meshFile object:", meshFile);
                        if (meshFile.threeMesh) {
                            // console.log("threeMesh is ready:", meshFile.threeMesh);
                        } else {
                            // console.warn("threeMesh is not ready:", meshFile);
                        }
                        $(document).trigger("MeshFileOpened", [meshFile]);
                    }
                });
